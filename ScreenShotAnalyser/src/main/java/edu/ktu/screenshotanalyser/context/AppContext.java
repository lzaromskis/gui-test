...
public class AppContext {
    public AppContext(File appFolder, File dataFolder, List<TestDevice> testDevices, SystemContext systemContext) throws IOException {
        ...
        for (var testDevice : testDevices) {
            var stateFiles = new File(testDevice
                                          .getFolder()
                                          .getAbsolutePath() + "/" + appFolder.getName() + "/states/").listFiles(pathelement -> pathelement
                .getAbsolutePath()
                .endsWith("." + stateFileExtension));

            if (null != stateFiles) {
                for (var stateFile : stateFiles) {
                    var stateImage = new File(stateFile
                                                  .getAbsolutePath()
                                                  .replaceAll(stateFileExtension + "$", "png")
                                                  .replace("state_", "screen_"));

                    if ((stateImage.exists()) && (stateImage.isFile()) && (stateImage.length() > 0)) {
                        this.states.add(new State("", this, stateImage, stateFile, testDevice));
                    } else {
                        stateImage = new File(stateFile
                                                  .getAbsolutePath()
                                                  .replaceAll(stateFileExtension + "$", "jpg")
                                                  .replace("state_", "screen_"));
        ...
        var apkFiles = appFolder.listFiles(p -> p.isFile() && p
            .getName()
            .endsWith(".apk"));
