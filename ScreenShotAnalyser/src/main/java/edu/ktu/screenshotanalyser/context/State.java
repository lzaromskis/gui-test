...
public class State {
    ...
    private HashMap<Long, Control> getControls(DocumentContext document) {
        ...
        @SuppressWarnings("unchecked") List<HashMap<String, Object>> viewObjects = document.read("$.views", List.class);

        for (var viewObject : viewObjects) {
            HashMap<String, Object> m = viewObject;

            var view = new Control(this,
                                   getText(m, "text"),
                                   getText(m, "content_description"),
                                   getBounds(m),
                                   (Integer) m.get("parent"),
                                   (Integer) m.get("temp_id"),
                                   (String) m.get("class"),
                                   (boolean) m.get("visible"),
                                   getText(m, "signature"));
    ...
    private Rect getBounds(HashMap<String, Object> view) {
        @SuppressWarnings("unchecked") var b = (List<List<Integer>>) view.get("bounds");
        var r = new Rect();
